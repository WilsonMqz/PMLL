...
lafter_datasets = ['DescribableTextures',  'EuroSAT', 'OxfordFlowers', 'OxfordPets', 'StanfordCars', 'Food101', 'SUN397', 'UCF101', 'ImageNetR', 'ImageNetSketch',
...
def setup_lafter_training_utils_StepLR_t(args, model):
    model = model.cuda()
    model = model.float()
    params = list()
    for key, value in model.named_parameters():
    ...
    optimizer = optim.AdamW(optimizer_grouped_parameters, lr=args.lr_t, betas=(0.9, 0.999))
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, args.mile_stones, 0.60)
    # scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.90)
    criteria = LabelSmoothingCrossEntropy()
    return optimizer, scheduler, criteria

def setup_lafter_training_utils_StepLR_f(args, model):
    model = model.cuda()
    model = model.float()
    params = list()
    for key, value in model.named_parameters():
    ...
    optimizer = optim.AdamW(optimizer_grouped_parameters, lr=args.lr_f, betas=(0.9, 0.999))
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, args.mile_stones, 0.60)
    # scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.90)
    criteria = LabelSmoothingCrossEntropy()
    return optimizer, scheduler, criteria
...
def test_prompting(teloader, model, args):
    ...
    for i, inputs in enumerate(tqdm(teloader, desc="Test")):
        labels = inputs['label']
        inputs = inputs['img']
    ...
    conceal_cls = cls_acc[medium1:medium2]
    conceal_cls_acc = conceal_cls.sum() / (len(conceal_cls))
    model.eval()
    return top1.avg * 100, conceal_cls_acc * 100
...
text_cls_epochs = {
    ...
    'OxfordPets': 600, # 600 for txt_cls
    'StanfordCars': 600, # 600 for txt_cls
